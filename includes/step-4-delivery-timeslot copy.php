    ...
    <div class="pro_delivery">
      <div class="pro_delivery_warper">
        ...
        <div id="arrival_window" class="pro_col">
          ...
          <div id="arrival_window_inner1" style="display:none">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    timeSlots.forEach(slot => {
        slot.addEventListener('click', function () {
            ...
            // Optionally show next button if required
            const nextBtn = document.getElementById('next_button');
            if (nextBtn) nextBtn.style.display = 'hidden';
        });
    });
});
</script>
...
<script>
/* document.addEventListener('DOMContentLoaded', function () {
    const dateInput = document.querySelector('#supply_timeslot');
    const calendarContainer = document.querySelector('#calendar_container_supply');
    const arrivalWindow = document.querySelector('#arrival_window_inner1');

    if (!dateInput || !calendarContainer) return;

    // Initialize Flatpickr inline inside calendar_container_supply
    flatpickr(dateInput, {
        minDate: 'today',
        dateFormat: 'Y-m-d',
        disableMobile: true,
        clickOpens: false, // Don't open as popup
        inline: true,
    ...
    // NO scrollIntoView – screen won't move
}); */
</script>
...
/* document.addEventListener('DOMContentLoaded', function () {
    ...
    // Initialize flatpickr INLINE under the input
    flatpickr(dateInput, {
        minDate: 'today',
        dateFormat: 'Y-m-d',
        disableMobile: true,
        clickOpens: false, // Disable popup
        inline: true,
        appendTo: calendarContainer,
        onChange: function () {
            if (arrivalWindow) arrivalWindow.style.display = 'block';
    ...
    // Click to open inline calendar (scroll to it smoothly)
    dateInput.addEventListener('click', function () {
        calendarContainer.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });

    // Handle time slot selection (independent logic)
    const timeSlotElements = document.querySelectorAll('.time-slot-1');
    ...
    timeSlotElements.forEach(slot => {
        slot.addEventListener('click', function () {
            ...
            // Optional: store time if needed
            // document.querySelector('#selected_time_input').value = this.dataset.time;
